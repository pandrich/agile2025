  ...
  as.formula(
    paste(
      ...
      "(
        ...
        + clim
        + (1 + clim | country)
        + gp(lon, lat, cov = 'matern12', gr = TRUE)
      )"
    )
  )
)
...
clim_draws <- (
  bind_rows(clim_draws_nonranef, clim_draws_ranint, clim_draws_ns, clim_draws_sp)
  %>% mutate(
    model = factor(model, levels = c("No random effects", "Random intercept", "No spatial terms", "With spatial corr.")
    )
  )
)
```
