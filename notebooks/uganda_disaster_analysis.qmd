...
load_dhis_data <- function(filename) {
...
https://data.humdata.org/dataset/uga-iom-dtm-from-api

```{r}
disp_cols <- (
  read_csv(
    file.path(raw_data_dir, "migration", "uga-iom-dtm-from-api-admin-0-to-2.csv"),
    show_col_types = FALSE,
    n_max = 0
  )
  %>% names()
...
df_utci <- (
  read_csv(
    file.path(proc_data_dir, "utci_UGA_adm2.csv"),
    show_col_types = FALSE
  )
  %>% mutate(
...
```{r}
df_sgbv <- (
  load_dhis_data("DHIS2_GBV_Data.xlsx")
  %>% mutate(
    # The dataset of the geographic areas does not include separate district for cities 
    # so we need to condense district + city in the data (note that Fort Portal is the 
    # location of the seat of the Kabarole district
    # Terego district does not appear in the geometries and we assign it to Arua
    # as it used to be
    adm2_name = str_squish(str_remove_all(district, "District|City")),
    adm2_name = case_when(
      adm2_name == "Fort Portal" ~ "Kabarole",
      adm2_name == "Fort" ~ "Kabarole",
      adm2_name == "Terego" ~ "Arua",
      .default = adm2_name
    ),
  ...
  %>% group_by(adm2_name, date, variable, sex, age_bracket)
  %>% summarise(
    cases = sum(cases, na.rm = TRUE),
    .groups = "drop"
  )
  %>% arrange(adm2_name, date, variable, sex, age_bracket)
)
```

# Data processing

## Combine UTCI, geographies and population
