---
title: "Model main temperature effect on all sexual violence - adults, admin 2"
format: html
editor: visual
---
...
```{r}
# Settings
nb_name <-  "Model main temperature effect on all sexual violence - adults, admin 2"
...
# Var settings
clim_var <- "dev_value_pop_weighted_utci_max_above_p85_12m"
clim_label <- "dev_utci_above_p85"
viol_var <- "sv"
viol_label <- "all_viol"
pop_label <- "adults"
covars <- c(
...
sf_zwe <- (
  ...
  %>% mutate(
    adm2_name = case_when(
      adm2_name == "Beitbridge" ~ "Beitbridge Rural",
      adm2_name == "Bindura" ~ "Bindura Rural",
      adm2_name == "Centenary/ Muzarabani" ~ "Centenary",
      adm2_name == "Chegutu" ~ "Chegutu Rural",
      adm2_name == "Chipinge" ~ "Chipinge Rural",
      adm2_name == "Chiredzi" ~ "Chiredzi Rural",
      adm2_name == "Chiredzi Urban" ~ "Chiredzi Town",
      adm2_name == "Gokwe South Urban" ~ "Gokwe Centre",
      adm2_name == "Gwanda" ~ "Gwanda Rural",
      adm2_name == "Gweru" ~ "Gweru Rural",
      adm2_name == "Kwekwe" ~ "Kwekwe Rural",
      adm2_name == "Marondera" ~ "Marondera Rural",
      adm2_name == "Marondera Urban" ~ "Marondera",
      adm2_name == "Masvingo" ~ "Masvingo Rural",
      adm2_name == "Mhondoro-Ngezi" ~ "Mhondoro Ngezi",
      adm2_name == "Mount Darwin" ~ "Mt Darwin",
      adm2_name == "Mutare" ~ "Mutare Rural",
      adm2_name == "Shurugwi" ~ "Shurugwi Rural",
      adm2_name == "Shurugwi Town" ~ "Shurugwi Urban",
      adm2_name == "Uzumba Maramba Pfungwe" ~ "Ump",
      adm2_name == "Victoria Falls" ~ "Vic Falls",
      adm2_name == "Zvishavane" ~ "Zvishavane Rural",
      adm2_name == "Zvishavane Urban" ~ "Zvishavane",
      .default = adm2_name
    )
  )
  %>% arrange(adm2_name)
...
df_clim_viol_summ <- (
  ...
  %>% summarise(
    ...
    partner = sum(live_with_partner * sample_weight, na.rm = TRUE) / sum(sample_weight, na.rm = TRUE),
    school = sum(school_level * sample_weight, na.rm = TRUE) / sum(sample_weight, na.rm = TRUE),
    sv = sum(sv * sample_weight, na.rm = TRUE) / sum(sample_weight, na.rm = TRUE),
    .groups = "drop"
  )
  %>% merge(
    sf_all,
    by = c("country", "adm2_name"),
    all.x = TRUE
  )
  %>% st_as_sf()
)
...
sf_vacs <- (
  ...
  %>% filter(
    ...
    adm2_name %in% (df_clim_viol$adm2_name %>% unique())
  )
  %>% mutate(
    centroid = st_centroid(geometry),
...
df_model <- (
  ...
  %>% na.omit()
  %>% merge(
    sf_vacs %>% as.data.frame() %>% select(country, adm1_name, adm2_name, lon, lat),
    by = c("country", "adm2_name"),
    all.x = TRUE
  )
  %>% mutate(
  ...
  %>% group_by(adm1_name)
  %>% mutate(
    clim_mean = mean(clim, na.rm = TRUE)
  )
  %>% ungroup()
  %>% mutate(
    clim_within = clim - clim_mean,
  )
)
```
...
rownames(W) <- sf_vacs$adm2_name
colnames(W) <- sf_vacs$adm2_name
levs <- levels(df_model$adm2_name)
W_ord <- W[levs, levs]
```
...
#| fig-height: 2.5
(
  plot_single_continuous_marginal_by_group(fit_nonranef, df_model, cov = "clim_within")
  + scale_y_continuous(
    limits = c(0, 0.35),
    labels = scales::percent_format(accuracy = 1)
  )
...
## With country/climate interaction
...
```{r}
sp <- rstan::get_sampler_params(fit_int$fit, inc_warmup = FALSE)
ebfmi_per_chain <- sapply(sp, function(chain_mat) {
  ener <- chain_mat[,"energy__"]
  mean(diff(ener)^2) / var(ener)
})
...
```{r}
plot(fit_int, variable = "^b_rural", regex = TRUE, nvariables = 3)
```
...
df_base_sv_by_country_int_stats <- (
  df_base_sv_by_country_int
  %>% group_by(country)
  %>% summarise(
    median_p_baseline_sv = median(p_baseline_sv, na.rm = TRUE),
...
(
  ggplot(
    data = df_base_sv_by_country_int,
    mapping = aes(
      y = fct_reorder(country, p_baseline_sv, .fun = ~ -median(.)),
      x = p_baseline_sv
    )
  ...
  + geom_segment(
      data = df_base_sv_by_country_int_stats,
      aes(y = country, yend = country, x = lower2_p, xend = upper2_p),
      linewidth = 1,
      alpha = 0.4,
      color = "#A63603FF",
...
#| fig-width: 5
#| fig-height: 3
plot_bin_marginal(fit_int, df_model, covs = bin_covs)
ggsave(
  path = fig_dir,
  filename = paste0("marginal_prob_bin_main_effect_", clim_label, "_", pop_label, "_", viol_label, "_int.png")
)
```
...
#| fig-width: 6.5
#| fig-height: 3
plot_mono_marginal(fit_int, df_model, covs = mono_covs)
ggsave(
  path = fig_dir,
  filename = paste0("marginal_prob_mono_main_effect_", clim_label, "_", pop_label, "_", viol_label, "_int.png")
)
```
...
#| fig-height: 3
(
  plot_continuous_marginal(fit_int, df_model, covs = cont_covs)
  + scale_y_continuous(
    limits = c(0, 0.6),
    labels = scales::percent_format(accuracy = 1)
  )
...
ggsave(
  path = fig_dir,
  filename = paste0("marginal_prob_clim_main_effect_", clim_label, "_", pop_label, "_", viol_label, "_int.png")
)
```
...
#| fig-height: 2.5
(
  plot_single_continuous_marginal_by_group(fit_int, df_model, cov = "clim")
  + scale_y_continuous(
    limits = c(0, 0.35),
    labels = scales::percent_format(accuracy = 1)
  )
...
ggsave(
  path = fig_dir,
  filename = paste0("marginal_prob_clim_main_effect_by_country_", clim_label, "_", pop_label, "_", viol_label, "_int.png")
)
```
...
#| fig-width: 4
#| fig-height: 3
plot_calibration(fit_int, df_model, outcome_var = viol_var)
ggsave(
  path = fig_dir,
  filename = paste0("calibration_plot_main_effect_", clim_label, "_", pop_label, "_", viol_label, "_int.png")
)
```
...
ggsave(
  path = fig_dir,
  filename = paste0("moran_plot_obs_main_effect_", clim_label, "_", pop_label, "_", viol_label, "_int.png")
)
```
...
#| fig-width: 3.1
#| fig-height: 3
plot_moran(df_model, sf_vacs, fit = fit_int, residuals = TRUE)
ggsave(
  path = fig_dir,
  filename = paste0("moran_plot_res_main_effect_", clim_label, "_", pop_label, "_", viol_label, "_int.png")
)
```
...
#| fig-height: 2.5
(
  plot_ppc_rootogram(fit_int, df_model, outcome_var = viol_var)
  + theme(
    legend.position = "inside",
    legend.position.inside = c(0.92, 0.8)
  )
...
ggsave(
  path = fig_dir,
  filename = paste0("ppc_by_adm_main_effect_", clim_label, "_", pop_label, "_", viol_label, "_int.png")
)
```
...
ggsave(
  path = fig_dir,
  filename = paste0("clim_post_covariate_main_effect_", clim_label, "_", pop_label, "_", viol_label, "_int.png")
)
```
...
### Pre whitening of climate variable
...
form_pw <- bf(clim ~ car(W, gr = adm_name, type = "escar"))
# form_pw <- bf(clim ~ gp(lon, lat, cov = 'matern32', gr = TRUE))
...
pw_res <- (
  df_model
  %>% select(country, adm_name, clim)
  %>% add_residual_draws(fit_pw)
  %>% group_by(country, adm_name, .row)
  %>% summarise(
    clim = mean(.residual, na.rm = TRUE),
    .groups = "drop"
  )
  %>% mutate(
    clim = scale(clim)
...
```{r}
sp <- rstan::get_sampler_params(fit_pw_res$fit, inc_warmup = FALSE)
ebfmi_per_chain <- sapply(sp, function(chain_mat) {
  ener <- chain_mat[,"energy__"]
  mean(diff(ener)^2) / var(ener)
})
...
```{r}
plot(fit_pw_res, variable = "prior_b", regex = TRUE, nvariables = 3)
```

```{r}
plot(fit_pw_res, variable = "^b_rural", regex = TRUE, nvariables = 3)
```
...
#| fig-width: 5
#| fig-height: 3
plot_baseline_levels(fit_pw_res, lims = c(0, 0.3))
```
...
#| fig-width: 5
#| fig-height: 3
plot_bin_marginal(fit_pw_res, df_model_pw, covs = bin_covs)
ggsave(
  path = fig_dir,
  filename = paste0("marginal_prob_bin_main_effect_", clim_label, "_", pop_label, "_", viol_label, "_pw.png")
)
```
...
#| fig-width: 8
#| fig-height: 3
plot_mono_marginal(fit_pw_res, df_model_pw, covs = c("rel_age_ord", "school_level"))
ggsave(
  path = fig_dir,
  filename = paste0("marginal_prob_mono_main_effect_", clim_label, "_", pop_label, "_", viol_label, "_pw.png")
)
```
...
#| fig-height: 3
(
  plot_continuous_marginal(fit_pw_res, df_model_pw, covs = c("clim"))
  + scale_y_continuous(
    limits = c(0, 0.35),
    labels = scales::percent_format(accuracy = 1)
  )
...
ggsave(
  path = fig_dir,
  filename = paste0("marginal_prob_clim_main_effect_", clim_label, "_", pop_label, "_", viol_label, "_pw.png")
)
```
...
#| fig-height: 3.2
(
  plot_mono_marginal_by_group(fit_pw_res, df_model_pw, covs = c("rel_age_ord", "school_level"), dodge = 0.6)
  + theme(
    legend.position = "top"
  )
)
ggsave(
  path = fig_dir,
  filename = paste0("marginal_prob_mono_main_effect_by_country_", clim_label, "_", pop_label, "_", viol_label, "_pw.png")
)
```
...
#| fig-height: 2.5
(
  plot_single_continuous_marginal_by_group(fit_pw_res, df_model_pw, cov = "clim")
  + scale_y_continuous(
    limits = c(0, 0.35),
    labels = scales::percent_format(accuracy = 1)
  )
...
ggsave(
  path = fig_dir,
  filename = paste0("marginal_prob_clim_main_effect_by_country_", clim_label, "_", pop_label, "_", viol_label, "_pw.png")
)
```
...
#| fig-width: 4
#| fig-height: 3
plot_calibration(fit_pw_res, df_model_pw, outcome_var = viol_var)
ggsave(
  path = fig_dir,
  filename = paste0("calibration_plot_main_effect_", clim_label, "_", pop_label, "_", viol_label, "_pw.png")
)
```
...
#| fig-width: 5
#| fig-height: 2.5
plot_moran(df_model_pw, sf_vacs, outcome_var = viol_var)
ggsave(
  path = fig_dir,
  filename = paste0("moran_plot_obs_main_effect_", clim_label, "_", pop_label, "_", viol_label, "_pw.png")
)
```
...
#| fig-width: 3.15
#| fig-height: 3
plot_moran(df_model_pw, sf_vacs, fit = fit_ns, residuals = TRUE)
ggsave(
  path = fig_dir,
  filename = paste0("moran_plot_res_main_effect_", clim_label, "_", pop_label, "_", viol_label, "_pw.png")
)
```
...
#| fig-height: 2.5
(
  plot_ppc_rootogram(fit_pw_res, df_model_pw, outcome_var = viol_var)
  + theme(
    legend.position = "inside",
    legend.position.inside = c(0.92, 0.8)
  )
...
ggsave(
  path = fig_dir,
  filename = paste0("ppc_by_adm_main_effect_", clim_label, "_", pop_label, "_", viol_label, "_pw.png")
)
```
...
```{r}
df_clim_pop_pw <- (
  as_draws_df(fit_pw_res)
  %>% select(clim_pop = b_clim)
)
df_clim_ranef_pw <- (
  as_tibble(ranef(fit_pw_res, summary = FALSE)$country[, , 2])
  %>% bind_cols(df_clim_pop_pw)
  %>% mutate(
    across(
      .cols = -clim_pop, 
      .fns = ~ (.) + clim_pop,
...
ggsave(
  path = fig_dir,
  filename = paste0("clim_post_covariate_main_effect_", clim_label, "_", pop_label, "_", viol_label, "_pw.png")
)
```
...
(
  ...
  + geom_segment(
    data = df_clim_ranef_plot_pw_stats,
    aes(y = country, yend = country, x = lower2_val, xend = upper2_val),
    linewidth = 1,
    alpha = 0.4,
    color = "#A63603FF",
...
ggsave(
  path = fig_dir,
  filename = paste0("clim_post_covariate_main_effect_by_country_", clim_label, "_", pop_label, "_", viol_label, "_pw.png")
)
```
...
bind_rows(
  ...
  get_calibration_scores(fit_ranint, df_model, outcome_var = viol_var, model_name = "Random Intercept"),
...
clim_draws <- (
  ...
  %>% mutate(
    model = factor(model, levels = c("No Random Effects", "Random Intercept", "With Random Effects", "With Spatial Corr.")
    )
  )
)
```
