...
build_weighted_utci_daily_country <- function(data_dir, iso3, rast_pop, adm_pop, adm_shapes) {
  ...
  for (i in seq_along(rast_files)) {
    ...
    dfs_utci_daily[[i]] <- (
      ...
      %>% mutate(
        ...
        utci_max = (result / popf_10_19) - 273.15
      )
    )
  }
  df_utci <- (
  ...
  df_utci %>% write_csv(file.path(proc_data_dir, paste0("age_weighted_utci_max_daily_", iso3, ".csv")))
  df_utci
}
```
...
for (i in seq_along(ages)) {
  rast_pops[[i]] <- (
    ...
    %>% rename(
      popf_10_19 = sprintf("global_f_%02d_2019_CN_1km_R2025A_v1", ages[[i]])
    )
  )
}
...
for (i in seq_along(ages)) {
  rast_pops[[i]] <- (
    ...
    %>% rename(
      popf_10_19 = sprintf("global_f_%02d_2017_CN_1km_R2025A_v1", ages[[i]])
    )
  )
}
...
for (i in seq_along(ages)) {
  rast_pops[[i]] <- (
    ...
    %>% rename(
      popf_10_19 = sprintf("global_f_%02d_2018_CN_1km_R2025A_v1", ages[[i]])
    )
  )
}
...
for (i in seq_along(ages)) {
  rast_pops[[i]] <- (
    ...
    %>% rename(
      popf_10_19 = sprintf("global_f_%02d_2015_CN_1km_R2025A_v1", ages[[i]])
    )
  )
}
...
for (i in seq_along(ages)) {
  rast_pops[[i]] <- (
    ...
    %>% rename(
      popf_10_19 = sprintf("global_f_%02d_2024_CN_1km_R2025A_v1", ages[[i]])
    )
  )
}
...
df_weighted_utci %>% write_csv(file.path(proc_data_dir, "age_weighted_utci_max_daily_all_countries.csv"))
```
