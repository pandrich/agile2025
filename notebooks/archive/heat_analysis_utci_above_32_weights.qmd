...
df_clim_viol <- (
  ...
  %>% group_by(country)
  %>% mutate(
    sample_weight = 5 * (sample_weight / max(sample_weight, na.rm = TRUE))
  )
)
```
...
#| fig-width: 7
#| fig-height: 3
plot_continuous_marginal(df_marg_cont_fem_all_viol, covar_label = "Climate factor", lims = c(0.02, 0.25))
