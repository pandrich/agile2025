...
#   alpha_sv <- -1    # sexual violence intercept
#   beta_cs <- 0.25      # effect of the climate variable on sexual violence
# 
...
```{r}
test_seeds[which.min(abs(test_clim - 0.5))]
```
### Interesting seeds
...
clim_draws_6 <- (
  ...
  %>% mutate(
    model = factor(model, levels = c("No Spatial Corr.", "Adm. Level Intercept", "With Spatial Corr.")
    )
  )
)
```
