...
```{r}
mean_corrs_reg <-  get_mean_corrs_multi_var(df_reg_corr, vars = vars, level = "region")
```
...
(
  ...
  + geom_text(
    data = subset(df_distr_year_plot, (mean_maln_2024 - mean_maln_2022) < -0.45),
    mapping = aes(
      x = mean_maln_2022,
      y = mean_maln_2024 + 0.1,
      label = district,
...
ggsave(
  path = fig_dir,
  filename = "viol_2022_vs_2024.png"
)
```
...
ggsave(
  path = fig_dir,
  filename = "violence_over_time_largest_increases.png"
)
```
