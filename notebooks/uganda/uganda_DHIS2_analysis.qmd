...
Data from: https://data.humdata.org/dataset/global-wfp-food-prices We process this data with the notebook `preprocess_food_prices.qmd` to extract a time series of the Maize (white) prices where we attempt to interpolate the missing values using neighboring districts.
...
#| fig-height: 3.5
min_year <- 2020
max_year <- max(year(df_ndvi$date))
...
df_utci <- (
  read_csv(
    file.path(
      proc_data_dir,
      "UTCI",
      "utci_max_daily_all_countries.csv"
    ),
    show_col_types = FALSE
  )
  %>% filter(
  ...
  %>% summarise(
    utci_max_above_32 = sum(utci_max > 32, na.rm = TRUE),
    utci_max_above_36 = sum(utci_max > 36, na.rm = TRUE),
    utci_max = max(utci_max, na.rm = TRUE),
    .groups = "drop"
  )
)
```
...
df_utci_pop_weighted <- (
  ...
  %>% summarise(
    weighted_utci_max_above_32 = sum(utci_max > 32, na.rm = TRUE),
    weighted_utci_max_above_36 = sum(utci_max > 36, na.rm = TRUE),
    weighted_utci_max = max(utci_max, na.rm = TRUE),
    .groups = "drop"
  )
)
```

### Age population weighted UTCI
...
df_utci_girl_weighted <- (
  ...
  %>% summarise(
    girl_weighted_utci_max_above_32 = sum(utci_max > 32, na.rm = TRUE),
    girl_weighted_utci_max_above_36 = sum(utci_max > 36, na.rm = TRUE),
    girl_weighted_utci_max = max(utci_max, na.rm = TRUE),
    .groups = "drop"
  )
)
```
...
df_spei_girl_weighted <- (
  ...
  %>% rename(
    district_code = adm_code,
    district = adm_name,
    girl_weighted_spei = spei
  )
  %>% select(-c(country, popf_10_19))
)
```
...
df_rain <- (
  ...
  %>% summarise(
    rainfall_anomalies = sum(rainfall > mean_rainfall + 2 * sd_rainfall, na.rm = TRUE),
...
df_rain_pop_weighted <- (
  ...
  %>% summarise(
    pop_weighted_rainfall_anomalies = sum(rainfall > mean_rainfall + 2 * sd_rainfall, na.rm = TRUE),
    pop_weighted_max_rainfall = max(rainfall, na.rm = TRUE),
    .groups = "drop"
  )
)
```
...
```{r}
df_rain_girl_weighted <- (
  read_csv(
    file.path(
      proc_data_dir,
      "rainfall",
      "age_weighted_rain_all_countries.csv"
    ),
    show_col_types = FALSE
  )
  %>% filter(
  ...
  %>% summarise(
    girl_weighted_rainfall_anomalies = sum(rainfall > mean_rainfall + 2 * sd_rainfall, na.rm = TRUE),
    girl_weighted_max_rainfall = max(rainfall, na.rm = TRUE),
    .groups = "drop"
  )
)
```
...
df_viol_all <- (
  ...
  %>% pivot_wider(
    ...
    names_from = c(sex, age_bracket),
    values_from = viol_cases,
    names_prefix = "viol_",
    names_sep = "_"
  )
  %>% mutate(
    viol_cases = rowSums(across(-c(district, date)), na.rm = TRUE)
  )
)
```
...
df_health_distr <- (
  ...
  #     sd_value_per_1000 > 0,
  #     (value_per_1000 - mean_value_per_1000),
  #     0
  #   )
  # )
  %>% mutate(
    dev_value = value - mean_value,
    dev_value_per_1000 = value_per_1000 - mean_value_per_1000
  )
  %>% select(
...
df_clim <- (
  df_utci
  %>% merge(
    df_utci_pop_weighted %>% select(-district),
    by = c("district_code", "date"),
    all.x = TRUE
  )
  %>% merge(
    df_utci_girl_weighted %>% select(-district),
    by = c("district_code", "date"),
    all.x = TRUE
  )
  %>% merge(
    df_spei %>% select(-district),
    by = c("district_code", "date"),
    all.x = TRUE
  )
  %>% merge(
    df_spei_pop_weighted %>% select(-district),
    by = c("district_code", "date"),
    all.x = TRUE
  )
  %>% merge(
    df_spei_girl_weighted %>% select(-district),
    by = c("district_code", "date"),
    all.x = TRUE
  )
  %>% merge(
    df_rain %>% select(-district),
    by = c("district_code", "date"),
    all.x = TRUE
  )
  %>% merge(
    df_rain_pop_weighted %>% select(-district),
    by = c("district_code", "date"),
    all.x = TRUE
  )
  %>% merge(
    df_rain_girl_weighted %>% select(-district),
    by = c("district_code", "date"),
    all.x = TRUE
  )
  %>% merge(
  ...
  %>% mutate(
    ...
    max_rainfall_density = max_rainfall / district_area,
    pop_weighted_max_rainfall_density = pop_weighted_max_rainfall / district_area,
    girl_weighted_max_rainfall_density = girl_weighted_max_rainfall / district_area
  )
  %>% select(-c(district_code, district_area, geometry))
)
```
...
## Calculate deviation from monthly mean by district
...
df_clim_distr_month <- (
  ...
  %>% pivot_longer(
    cols = -c(district, date, month),
    names_to = "variable",
    values_to = "value"
  )
  %>% group_by(district, variable, month)
  %>% mutate(
    mean_value = mean(value, na.rm = TRUE),
  )
  %>% ungroup()
...
## Calculate deviation from monthly mean by region
...
df_clim_reg_month <- (
  ...
  %>% group_by(region, date)
  %>% summarise(
    across(-c(district), ~ mean(., na.rm = TRUE)),
    .groups = "drop"
  )
  %>% pivot_longer(
    cols = -c(region, date),
  ...
  %>% group_by(region, month, variable)
  %>% mutate(
    mean_value = mean(value, na.rm = TRUE),
  )
  %>% ungroup()
...
(
  df_reg
  %>% filter(
    variable == "dev_value_weighted_utci_max_above_32"
  )
  # %>% filter(
  #   region != "Rwenzori"
  # )
...
```{r}
# Video parameters
video_start_date <- as.Date("2024-04-30")   
n_frames        <- 60                   
step            <- 1                      
fps             <- 2                   
png_width       <- 5                     
...
(
  df_reg
  %>% filter(
    variable == "dev_value_pop_weighted_spei"
  )
  # %>% filter(
  #   region != "Rwenzori"
  # )
...
(
  df_reg
  %>% filter(
    variable == "value_per_1000_maln_cases"
  )
  # %>% filter(
  #   region != "Rwenzori"
  # )
...
(
  df_reg
  %>% filter(
    variable == "dev_value_per_1000_viol_cases"
  )
  # %>% filter(
  #   region != "Rwenzori"
  # )
...
df_distr_month_summary <- (
  ...
  %>% filter(
    variable %in% c(
      "value_girl_weighted_utci_max_above_32",
      "value_girl_weighted_spei",
      "value_girl_weighted_max_rainfall",
      "value_per_1000_maln_cases",
      "value_per_1000_viol_cases"
    )
  )
  %>% group_by(month = lubridate::month(date, label = TRUE, abbr = TRUE), variable)
  %>% summarise(
...
(
  ...
  %>% ggplot(aes(x = month, y = dev_value, fill = variable))
  + geom_col()
  + facet_wrap(
    ~ variable,
    ncol = 1,
    ...
    labeller = as_labeller(c(
      "value_per_1000_viol_cases" = "Viol. per 1000",
      "value_per_1000_maln_cases" = "Maln. per 1000",
      "value_girl_weighted_utci_max_above_32" = "UTCI",
      "value_girl_weighted_spei" = "SPEI",
      "value_girl_weighted_max_rainfall" = "Rainfall"
    )),
    strip.position = "left"
  )
  + scale_fill_manual(
    values = c(
      "value_per_1000_viol_cases" = col_palette[1],
      "value_per_1000_maln_cases" = col_palette[2],
      "value_girl_weighted_utci_max_above_32" = col_palette[4],
      "value_girl_weighted_spei" = col_palette[6],
      "value_girl_weighted_max_rainfall" = col_palette[8]
    )
  )
  + theme(
    panel.background = element_blank(),
...
```{r}
cor(
  df_distr_month_summary %>% filter(variable == "value_per_1000_viol_cases") %>% pull(dev_value),
  df_distr_month_summary %>% filter(variable == "value_girl_weighted_utci_max_above_32") %>% pull(dev_value)
)
```
...
cor(
  df_distr_month_summary %>% filter(variable == "value_per_1000_viol_cases") %>% pull(dev_value),
  -(df_distr_month_summary %>% filter(variable == "value_girl_weighted_spei") %>% pull(dev_value))
)
```
...
cor(
  df_distr_month_summary %>% filter(variable == "value_per_1000_viol_cases") %>% pull(dev_value),
  df_distr_month_summary %>% filter(variable == "value_per_1000_maln_cases") %>% pull(dev_value)
)
```
...
cor(
  df_distr_month_summary %>% filter(variable == "value_per_1000_viol_cases") %>% pull(dev_value),
  df_distr_month_summary %>% filter(variable == "value_girl_weighted_max_rainfall") %>% pull(dev_value)
)
```
...
(
  df_distr 
  %>% filter(variable %in% c("value_per_1000_maln_cases", "value_per_1000_viol_cases"))
  %>% pivot_wider(
    id_cols = c(district, date),
    names_from = "variable",
    values_from = "value"
  ...
  %>% mutate(
    value_per_1000_viol_cases = lead(value_per_1000_viol_cases, n = 0)
  )
  %>% ggplot(
    aes(x = value_per_1000_maln_cases, y = value_per_1000_viol_cases)
  )
...
cols_to_plot <- c(
  ...
  # "dev_value_utci_days_above_32_daily_max",
  "dev_value_girl_weighted_utci_max_above_32",
  # "dev_value_spei",
  "dev_value_girl_weighted_spei",
  # "dev_value_rainfall_density",
  "dev_value_pop_weighted_max_rainfall_density",
  # "value_w_crop",
  # "value_maln_cases_per_1000",
  # "value_viol_cases_per_1000"
  # "dev_value_maln_cases_per_1000",
  "dev_value_per_1000_maln_cases",
  # "dev_value_viol_cases_per_1000"
  "dev_value_per_1000_viol_cases"
)
...
(
  ...
  + facet_wrap(
    ...
    labeller = as_labeller(c(
      ...
      "dev_value_per_1000_viol_cases" = "Viol. per 1000",
      "dev_value_per_1000_maln_cases" = "Maln. per 1000",
      "dev_value_girl_weighted_utci_max_above_32" = "UTCI",
      "dev_value_girl_weighted_spei" = "SPEI",
      "dev_value_pop_weighted_max_rainfall_density" = "Rainfall"
    )),
    strip.position = "left"
  )
  + scale_x_date(
...
(
  ...
  %>% filter(
    variable %in% c(
      "value_per_1000_viol_Female_5-9Yrs", 
      "value_per_1000_viol_Female_10-19Yrs",
      "value_per_1000_viol_Female_20+Yrs"
    )
...
p1 <- (
  ...
  %>% filter(
    variable == "dev_value_per_1000_maln_cases"
  #   # district %in% distr_to_consider
  #   district %in% elgon_region
  )
  %>% group_by(region, date, variable)
...
p2 <- (
  ...
  # )
  %>% ggplot(
    aes(x = date, y = value, color = variable, linetype = region)
  )
  + geom_line(
    linewidth = 1.2,
  )
  ...
  + paletteer::scale_colour_paletteer_d(
    "calecopal::sbchannel",
    labels = c(
      "value_per_1000_viol_Female_5-9Yrs" = "5-9",
      "value_per_1000_viol_Female_10-19Yrs" = "10-19",
      "value_per_1000_viol_Female_20+Yrs" = "20+"
    )
  )
  + theme(
    panel.background = element_blank(),
...
(
  df_distr
  %>% filter(variable == "value_per_1000_viol_cases")
  %>% mutate(
    region = if_else(
      district %in% elgon_region,
      "Elgon",
  ...
  %>% ggplot(
    aes(x = date, y = value, linetype = region)
  )
  + geom_line(
    linewidth = 1.2,
    color = "#61599DFF"
  ...
  + theme(
    ...
    legend.text = element_text(size = 15),
    legend.position = "inside",
    legend.position.inside = c(0.21, 0.78),
    axis.ticks.x = element_line(color = c(rep("black", num_years), rep(NA, num_years + 1)))
  )
  + labs(
    x = "Date",
...
p1 <- (
  ...
  %>% group_by(date)
  %>% summarise(
    region_area = sum(district_area_km2, na.rm = TRUE),
    region_flooded_area = sum(flooded_area_km2, na.rm = TRUE),
    region_flooded_frac = region_flooded_area / region_area
  )
  %>% ungroup()
  %>% group_by(
    date = lubridate::ceiling_date(date, "month") - lubridate::days(1)
  ...
  %>% summarise(
    max_flooded_area = max(region_flooded_area, na.rm = TRUE),
    max_flooded_frac = max(region_flooded_frac, na.rm = TRUE),
    .groups = "drop"
  )
  %>% ggplot()
  + geom_line(
    aes(x = date, y = max_flooded_frac),
    color = "#4C6FA1FF",
    linewidth = 1.2
  )
  + scale_x_date(
    breaks = c(as.Date(paste0(c(years, years[length(years)] + 1), "-01-01")), as.Date(paste0(years, "-07-01"))),
    labels = c(rep(c(""), num_years + 1), rep(years)),
    limits = c(min(df_reg$date), max(df_reg$date))
  )
  + scale_y_continuous(
...
p2 <- (
  ...
  + scale_color_manual(
    values = c(
      "value_per_1000_viol_Female_10-19Yrs" = "#C36377FF",
      "value_per_1000_viol_Female_20+Yrs" = "#F2AF4AFF"
    ),
    labels = c(
      "value_per_1000_viol_Female_10-19Yrs" = "10-19",
      "value_per_1000_viol_Female_20+Yrs" = "20+"
...
ggsave(file.path(fig_dir, "viol_cases_over_time_elgon_sep_age.png"))
```
