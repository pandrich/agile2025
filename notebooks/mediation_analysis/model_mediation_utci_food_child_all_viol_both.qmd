---
title: "Model food insecurity and utci impact on all sexual violence - all female"
format: html
editor: visual
---
...
```{r}
# Settings
nb_name <-  "Model food insecurity and utci impact on all sexual violence - all female"
...
df_viol <- (
  readRDS(
    file.path(model_data_dir, "merged_df_south_heat.rds")
  )
  %>% rename_with(str_to_lower)
  %>% filter(!is.na(hdate_vf))
  %>% mutate(
    sample_weight = 1E-3 * sampleweight,
    female = as.integer(sex == 2),
    rural = as.integer(rural_urban == 2),
    date = ceiling_date(as.Date(hdate_vf, format = "%Y-%m-%d"), "month") - days(1),
    adm_name = case_when(
      region == "Botha-Botha" ~ "Butha-Buthe",
      region == "karas" ~ "//Karas",
      region == "caprivi" ~ "Zambezi",
      region == "Harare" ~ "Harare Chitungwiza",
      str_detect(region, "Mash") ~ str_replace(region, "Mash", "Mashonaland"),
      str_detect(region, "Mat") ~ str_replace(region, "Mat", "Matabeleland"),
      .default = str_to_title(region)
    )
  )
  %>% select(
    date,
    ...
    # married,
    # food_insec
    in_school = school_enrol,
    education_ever = educationever,
    sv_np = any_sv_12m
  )
)
```
...
df_clim_viol <- (
  df_viol
  %>% merge(
    df_clim %>% select(country, adm_name, date, !!sym(clim_var)),
    by = c(
      "country", 
      "adm_name",
      "date"
  ...
  %>% merge(
    df_food_prices %>% select(country, adm_name, date, food_price_above_1s_12m),
    by = c(
      "country",
      "adm_name",
      "date"
...
#| fig-width: 6
#| fig-height: 2.5
plot_mono_marginal_by_group(fit_nf, df_model, covs = c("rel_age_ord"), dodge = 0.6)
ggsave(
  path = fig_dir,
...
#| fig-height: 3
(
  plot_mono_marginal_by_group(fit, df_model, covs = c("rel_age_ord", "food_price_above_1s_12m"), dodge = 0.6)
  + theme(
    legend.position = "top"
  )
...
df_clim_diff <- (
  (df_clim_ranef_nf - df_clim_ranef)
  %>% pivot_longer(
    cols = everything(),
    names_to = "country",
    values_to = "value"
...
df_comp <- (
  bind_cols(df_clim_pop_nf, df_clim_pop)
  %>% mutate(
...
#| fig-width: 6
#| fig-height: 2.5
plot_mono_marginal_by_group(fit_test_fi_nf, df_model_fi, covs = c("rel_age_ord"), dodge = 0.4)
```
