...
# Var settings
food_ins_var <- "food_price_above_1s_12m"
...
(
  ...
  %>% summarise(
    num_entries = n(),
    mean_food_price_above_1sd = mean(food_price_above_1sd, na.rm = TRUE),
    sd_food_price_above_1sd = sd(food_price_above_1sd, na.rm = TRUE),
    .groups = "drop"
  )
  %>% filter(
    num_entries >= 40
...
(
  ...
  %>% summarise(
    num_entries = n(),
    mean_food_price_above_1sd_12m = mean(food_price_above_1sd_12m, na.rm = TRUE),
    sd_food_price_above_1sd_12m = sd(food_price_above_1sd_12m, na.rm = TRUE),
    .groups = "drop"
  )
  %>% filter(
    num_entries >= 40
  ...
  %>% ggplot(
    aes(
      x = clim,
      y = mean_food_price_above_1sd_12m,
      color = country
    )
  )
  + geom_point()
...
(
  ...
  %>% mutate(
    ...
    food_price = as.numeric(food_price_above_1s_12m)
  )
  %>% group_by(country, clim)
  %>% summarise(
    median_food_price = mean(food_price, na.rm = TRUE),
    .groups = "drop"
  )
  %>% ungroup()
  %>% ggplot()
  ...
  as.formula(
    paste(
      ...
      "(
        ...
        + mo(clim)
        + (1 | country)
        + (-1 + mo(food_price_above_1s_12m)| country)
        + (-1 + mo(clim)| country)
        + car(W, gr = adm_name, type = 'escar')
      )"
    )
...
writeLines(stancode_ranef, file.path(mod_dir, "full_model_spei_1sd_food_adult_all_viol_ranef.stan"))
mod_ranef <- cmdstan_model(file.path(mod_dir, "full_model_spei_1sd_food_adult_all_viol_ranef.stan"))
```
...
#| fig-width: 5
#| fig-height: 2.5
pp_check(fit_ranef, resp = "foodpriceabove1s12m")
```
...
d0 <- setNames(drought_bounds$min_drought, drought_bounds$country)
...
```{r}
newdata_base_ranef <- df_model %>% select(-food_price_above_1s_12m)
newdata_d1_ranef <- (
  newdata_base_ranef
  %>% mutate(
    clim = d1[country]
...
mean_pp_fp_d0_by_country <- sapply(
  ...
  function(country_idxs) {
    if (length(country_idxs) == 1) {
      ...
      rowMeans(pp_fp_d0_ranef[, country_idxs])
    }
  }
)
...
mean_pp_fp_d1_by_country <- sapply(
  ...
  function(country_idxs) {
    if (length(country_idxs) == 1) {
      ...
      rowMeans(pp_fp_d1_ranef[, country_idxs])
    }
  }
)
```
