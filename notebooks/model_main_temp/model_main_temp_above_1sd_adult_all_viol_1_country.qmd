...
(
  ...
  + theme(
    ...
    legend.position.inside = c(0.942, 0.81)
  )
  + labs(
    y = "Counts (norm.)",
    x = "Excess hot days",
    title = "Climate variable distribution"
  )
)

ggsave(
  path = fig_dir,
  filename = paste0("climate_distribution_", clim_label, "_", pop_label, "_", viol_label, ".png")
)
```
...
country_to_plot <- "Zimbabwe"

(
  ggplot()
  + geom_histogram(
    data = df_clim %>% filter(country == country_to_plot),
    mapping = aes(
      y = after_stat(ncount),
      x = !!sym(clim_var),
      fill = "clim"
  ...
  + geom_histogram(
    data = df_clim_viol %>% filter(country == country_to_plot),
    mapping = aes(
      y = after_stat(ncount),
      x = !!sym(clim_var),
      fill = "clim_viol"
...
ggsave(
  path = fig_dir,
  filename = paste0("climate_distribution_", clim_label, "_", pop_label, "_", viol_label, "_", country_to_plot, ".png")
)
```
