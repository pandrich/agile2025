...
clim_var <- "dev_value_pop_weighted_temp_2m_max_above_p95_12m"
clim_label <- "temp_max_above_p95"
viol_var <- "sv"
viol_label <- "all_viol"
...
(
  ...
  + theme(
    plot.margin = margin(l = 20, r = 20, t = 5, b = 5)
  )
)
ggsave(
  path = fig_dir,
...
#| fig-width: 4
#| fig-height: 3
plot_mono_marginal(fit_ranint, df_model, covs = mono_covs)
ggsave(
  path = fig_dir,
...
#| fig-height: 3.2
(
  plot_mono_marginal_by_group(fit_ranint, df_model, covs = mono_covs, dodge = 0.6)
)
ggsave(
  path = fig_dir,
...
#| fig-width: 4
#| fig-height: 3
plot_mono_marginal(fit_pw_res, df_model_pw, covs = mono_covs)
ggsave(
  path = fig_dir,
...
#| fig-height: 3.2
(
  plot_mono_marginal_by_group(fit_pw_res, df_model_pw, covs = mono_covs, dodge = 0.6)
)
ggsave(
  path = fig_dir,
...
bind_rows(
  ...
  get_calibration_scores(fit_pw_res, df_model, outcome_var = viol_var, model_name = "Pre-whitened model")
)
```
