...
proc_data_dir <- root("data/processed/VACS_clean")
vacs_data_dir <- file.path(raw_data_dir, "VACS", "with_urban_rural")
...
vacs_ken <- (
  ...
  %>% mutate(
    ...
    country = "Kenya",
    female = if_else(sex == "2", 1, 0),
    adm1_name = str_to_title(county_name),
    adm2_name = NA,
    adm2_code = NA,
    hh_electricity = case_when(
      h7a == 1 ~ 1,
    ...
    sv_np_1 = case_when(
      ...
      (q605 > 88 & q612 > 88) ~ NA,
      (sv_1 == 1 & (is.na(q605) & is.na(q612))) ~ NA,
      (sv_1 == 1 & (q605 %in% c(5:16, 20:88) | q612 %in% c(5:16, 20:88))) ~ 1,
      .default = 0
    ),
    sv_ip_1 = case_when(
      is.na(sv_1) ~ NA,
      (q605 > 88 & q612 > 88) ~ NA,
      (sv_1 == 1 & (is.na(q605) & is.na(q612))) ~ NA,
      (sv_1 == 1 & (q605 %in% c(1:4, 17:19) | q612 %in% c(1:4, 17:19))) ~ 1,
      .default = 0
    ),
    q700a = if_else(
      is.na(q700a),
    ...
    sv_np_2 = case_when(
      ...
      (q705 > 88 & q712 > 88) ~ NA,
      (sv_2 == 1 & (is.na(q705) & is.na(q712))) ~ NA,
      (sv_2 == 1 & (q705 %in% c(5:16, 20:88) | q712 %in% c(5:16, 20:88))) ~ 1,
      .default = 0
    ),
    sv_ip_2 = case_when(
      is.na(sv_2) ~ NA,
      (q705 > 88 & q712 > 88) ~ NA,
      (sv_2 == 1 & (is.na(q705) & is.na(q712))) ~ NA,
      (sv_2 == 1 & (q705 %in% c(1:4, 17:19) | q712 %in% c(1:4, 17:19))) ~ 1,
      .default = 0
    ),
    q800a = if_else(
      is.na(q800a),
    ...
    sv_np_3 = case_when(
      ...
      (q805 > 88 & q813 > 88) ~ NA,
      (sv_3 == 1 & (is.na(q805) & is.na(q813))) ~ NA,
      (sv_3 == 1 & (q805 %in% c(5:16, 20:88) | q813 %in% c(5:16, 20:88))) ~ 1,
      .default = 0
    ),
    sv_ip_3 = case_when(
      is.na(sv_3) ~ NA,
      (q805 > 88 & q813 > 88) ~ NA,
      (sv_3 == 1 & (is.na(q805) & is.na(q813))) ~ NA,
      (sv_3 == 1 & (q805 %in% c(1:4, 17:19) | q813 %in% c(1:4, 17:19))) ~ 1,
      .default = 0
    ),
    q900a = if_else(
      is.na(q900a),
    ...
    sv_np_4 = case_when(
      ...
      (q905 > 88 & q912 > 88) ~ NA,
      (sv_4 == 1 & (is.na(q905) & is.na(q912))) ~ NA,
      (sv_4 == 1 & (q905 %in% c(5:16, 20:88) | q912 %in% c(5:16, 20:88))) ~ 1,
      .default = 0
    ),
    sv_ip_4 = case_when(
      is.na(sv_4) ~ NA,
      (q905 > 88 & q912 > 88) ~ NA,
      (sv_4 == 1 & (is.na(q905) & is.na(q912))) ~ NA,
      (sv_4 == 1 & (q905 %in% c(1:4, 17:19) | q912 %in% c(1:4, 17:19))) ~ 1,
      .default = 0
    ),
    q1000a = if_else(
      is.na(q1000a),
    ...
    sv_5 = case_when(
      (q1000a == 2 & q1000b == 2) ~ 0,
      (q1000a == 1 | q1000b == 1) & (q1002 == 2) ~ 0,
      (q1000a == 1 | q1000b == 1) & (q1002 == 1) ~ 1,
      .default = NA
    ),
    sv_np_5 = case_when(
      (q1000b == 2) ~ 0,
...
```{r}
df_vacs <- (
  bind_rows(vacs_moz, vacs_zwe, vacs_lso, vacs_nam, vacs_zmb, vacs_ken)
  %>% mutate(
  ...
  %>% ungroup()
  %>% mutate(
    sample_weight = sample_weight * (tot_weights / length(unique(country))) * (1 / country_weights)
  )
  %>% select(
    -c(tot_weights, country_weights)
  )
)
```
...
(
  ...
  %>% group_by(country)
  %>% summarise(
    perc_miss_sv = 100 * mean(is.na(sv))
  )
)
```
