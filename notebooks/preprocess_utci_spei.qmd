...
df_crops <- (
  ...
  %>% filter(
    country %in% c("Mozambique", "Zimbabwe", "Lesotho", "Namibia", "Zambia"),
    qc_flag == 0,
...
```{r}
df_clim_adm1 <- (
  bind_rows(df_clim_moz_adm1, df_clim_zwe_adm1, df_clim_lso_adm1, df_clim_nam_adm1, df_clim_zmb_adm1)
  %>% merge(
    df_crops,
...
df_clim_adm1_roll <- (
  ...
  %>% select(country, adm1_code, adm1_name, log_pop_density, date, crop_season, n_crop_months, spei_crop_sum_temp, dist_from_last_crop_season, contains("12m"))
)
```
...
(
  df_clim_adm1_roll
  # %>% filter(is.na(spei_crop_sum_temp))
  %>% filter(spei_crop_sum_temp == 0)
)
```
