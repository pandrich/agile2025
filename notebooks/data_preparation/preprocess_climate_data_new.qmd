---
title: "Preprocess Climate Data with Monthly Averages"
format: html
editor: visual
---
...
# Settings
nb_name <-  "Process daily climate data, monthly averages"
...
```{r}
df_utci_12m <- calculate_utci_12m_stats_monthly_ave(df_utci)
```
...
```{r}
df_utci_pop_weighted_12m <- calculate_utci_12m_stats_monthly_ave(df_utci_pop_weighted, weights = "pop_weighted")
```

### Girl Population Weighted UTCI

```{r}
df_utci_girl_weighted_12m <- calculate_utci_12m_stats_monthly_ave(df_utci_girl_weighted, weights = "girl_weighted")
```
...
```{r}
df_temp_12m <- calculate_temp_12m_stats_monthly_ave(df_temp)
```
...
```{r}
df_temp_pop_weighted_12m <- calculate_temp_12m_stats_monthly_ave(df_temp_pop_weighted, weights = "pop_weighted")
```
...
```{r}
df_temp_girl_weighted_12m <- calculate_temp_12m_stats_monthly_ave(df_temp_pop_weighted, weights = "girl_weighted")
```
...
```{r}
df_spei_12m <- calculate_spei_12m_stats_monthly_ave(df_spei)
```

### Population Weighted UTCI

```{r}
df_spei_pop_weighted_12m <- calculate_spei_12m_stats_monthly_ave(df_spei_pop_weighted, weights = "pop_weighted")
```

### Girl Population Weighted UTCI

```{r}
df_spei_girl_weighted_12m <- calculate_spei_12m_stats_monthly_ave(df_spei_girl_weighted, weights = "girl_weighted")
```
...
```{r}
df_clim_12m %>% write_csv(file.path(proc_data_dir, "processed_utci_temp_spei_daily_monthly_ave.csv"))
```
