...
```{r}
rast_temp_fine_moz <- terra::resample(rast_temp_moz[[1]], rast_pop_moz)
```
...
```{r}
rast_temp_weighted_moz <- rast_temp_fine_moz * rast_pop_moz
```
...
```{r}
df_weighted_temp_moz <- build_weighted_temp_daily_country("MOZ", country_map, rast_temp, rast_pop_moz, adm_pop_moz, sf_moz)
```
...
```{r}
df_weighted_temp_lso <- build_weighted_temp_daily_country("LSO", country_map, rast_temp, rast_pop_lso, adm_pop_lso, sf_lso)
```

### Namibia

```{r}
df_weighted_temp_nam <- build_weighted_temp_daily_country("NAM", country_map, rast_temp, rast_pop_nam, adm_pop_nam, sf_nam)
```
...
```{r}
df_weighted_temp_uga <- build_weighted_temp_daily_country("UGA", country_map, rast_temp, rast_pop_uga, adm_pop_uga, sf_uga)
```

### Kenya

```{r}
df_weighted_temp_ken <- build_weighted_temp_daily_country("KEN", country_map, rast_temp, rast_pop_ken, adm_pop_ken, sf_ken)
```
