...
df_clim <- (
  read_csv(
    file.path(proc_data_dir, "processed_utci_temp_spei_daily_monthly_ave.csv"),
    show_col_types = FALSE
  )
  %>% group_by(
    country,
...
```{r}
df_clim_fp %>% write_csv(file.path(model_data_dir, "climate_food_prices_new.csv"))
df_clim_fp_viol %>% write_csv(file.path(model_data_dir, "climate_food_prices_violence_new.csv"))
```
