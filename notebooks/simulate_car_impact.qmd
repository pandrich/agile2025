...
# Parameters
n_loc <- nrow(W)
N <- 50  # number of individual responders per location (we simplify by taking the same number everywhere)
adm_code <- rep(1:n_loc, each = N) # we create a vector of admin areas representing the location of the individual responders 
...
clim_adm <- beta_uc * u_adm + rnorm(n_loc, 0, clim_noise_sd)
...
sf_clim_3 <- bind_cols(sf_uga, clim_adm)
```
...
# Parameters
# 
# seed_new <- sample.int(1E7, size = 1)
# set.seed(seed_new)

# n_loc <- nrow(W)
# N <- 50
# adm_code <- rep(1:n_loc, each = N)
# 
# beta_uc <- 0.4
# clim_noise_sd <- 0.7
# 
...
# beta_us <- 2
# 
# # Build CAR precision matrix
# D <- diag(rowSums(W))
# rho_true <- 0.85
# sigma2 <- 1.0
# Q <- D - rho_true * W  
# 
# # Build unobserved confounder
# Sigma <- sigma2 * solve(Q)
# 
# u_adm <- MASS::mvrnorm(n = 1, mu = rep(0, n_loc), Sigma = Sigma)
# 
# # build clim variable
# clim_adm <- MASS::mvrnorm(n = 1, mu = rep(0, n_loc), Sigma = Sigma)
# 
# # Individual level data
# u <- u_adm[adm_code]
# clim <- clim_adm[adm_code]
