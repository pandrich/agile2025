...
for (ct in countries) {
  ...
  first_block <- sf_df_country %>% filter(date == dates_ct[[1]])
  nb_by_country[[ct]] <- poly2nb(first_block, queen = TRUE)
}
...
df_food_prices_adm_12m <- (
  ...
  %>% mutate(
    mean_price = mean(food_price_kg, na.rm = TRUE),
    sd_price = sd(food_price_kg, na.rm = TRUE),
  ...
  %>% ungroup()
  %>% mutate(
    food_price_dev_12m = (max_food_price_kg_12m - mean_price) / sd_price,
    food_stress_12m = as.integer(food_price_dev_12m > 1)
  )
  %>% as.data.frame()
  %>% select(-geometry)
)
