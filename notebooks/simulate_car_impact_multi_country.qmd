...
form_only_sp <- bf(
  sv ~ (
    1 
    # + gp(x1, x2, k = 10, cov = "matern32", gr = TRUE)
    + car(
      W,
      gr = adm_code,
      # type = "bym2"
...
adm_code <- rep(1:n_loc, each = n_ind)
alpha_sv <- -1    # sexual violence intercept
beta_cs <- 0.4      # effect of climate on sexual violence
...
beta_us <- 0.5    # effect of the unobserved variable on sexual violence
alpha_sv <- -1    # sexual violence intercept
beta_cs <- 0.7      # effect of the climate variable on sexual violence
...
gp_sigma_c <- 0.3
gp_lambda_c <- 1
r_u <- sqrt(3) * (d_mat / gp_lambda_u)
r_c <- sqrt(3) * (d_mat / gp_lambda_c)
Sigma_u <- (gp_sigma_u ** 2) * (1 + r_u) * exp(-r_u)
Sigma_c <- (gp_sigma_c ** 2) * (1 + r_c) * exp(-r_c)
