...
```{r}
build_utci_monthly_country <- function(iso3, rast_pop, adm_pop, adm_shapes) {
  years_folder <- list.files(file.path(raw_data_dir, "UTCI", "monthly", iso3))
  dfs <- list()
  i <- 0
    ...
    for (m in sprintf("%02d", 1:12)) {
      ...
      utci <- terra::rast(
        file.path(
          ...
          "monthly",
          iso3, 
          paste0("utci_monthly_", iso3, "_", year), 
...
```{r}
# rast_root_dir <- file.path(raw_data_dir, "UTCI/monthly/MOZ")
# rast_files <- unlist(lapply(root_dir, function(d) {
#   list.files(d, pattern = "^ECMWF_utci_monthly_stats", full.names = TRUE, recursive = TRUE)
# }))
# rast_list <- lapply(rast_files, function(f) {
#   r <- terra::rast(f)
#   r
# })
# utci_moz_rast <- do.call(c, rast_list)
# names(utci_moz_rast) <- paste0(
#   names(utci_moz_rast), 
#   "_", 
#   sapply(terra::time(utci_moz_rast), function(d) str_split_i(d, " ", 1))
# )
```
...
utci_moz_poly <- utci_moz_rast[[1]] %>% terra::as.polygons(values = FALSE, dissolve = FALSE)
```
...
utci_moz_weighted_cropped <- (
  utci_moz_weighted
  [[terra::names(utci_moz_rast) == "utci_days_above_32_daily_max"]]
  [[1]]
...
```{r}
utci_uga <- build_utci_monthly_country("UGA", rast_pop_uga_2022, adm_pop_uga_2022, sf_uga)
```
